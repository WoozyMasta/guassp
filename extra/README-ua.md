...
* SonarQube повинен мати налаштовану [ALM][] інтеграцію з вашим GitLab, а ім'я
  ключа інтеграції ви повинні вказати в `SONARQUBE_ALM_KEY`;
* Аутентифікація в SonarQube повинна відбуватися тільки через [ALM][] GitLab,
  інакше пошук користувачів буде порушено, права синхронізуються тільки для явно
  відповідного облікового запису, що прийшов з GitLab;
* Ви повинні мати можливість надати токен доступу з адміністративними привілеями
  для SonarQube в `SONARQUBE_TOKEN` для коректної роботи worker;
* Ви повинні створити службового користувача в GitLab, який повинен мати доступ
  ...
  * Токен користувача отриманий в SonarQube як `SONARQUBE_TOKEN`, що
    використовується в CI пайплайні для виконання аналізу та управління
    проектом.
